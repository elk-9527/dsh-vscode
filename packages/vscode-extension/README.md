...
| 命令 | 可用条件 | 带过去的内容 |
| --- | --- | --- |
| `DSH：把选中的代码带进对话` | 编辑器中有选区时 | 选中行的**正文**；文件已保存且无修改时另带文件链接 |
| `DSH：把当前文件带进对话` | 已打开文件时 | 已保存文件由 DSH 按链接读取；未保存或已修改文件直接带入编辑器当前正文 |

带过去的内容会显示为输入框上方的一个小块，点击 `×` 可以移除；发送之后，
该消息气泡中仍保留当时带入的清单。

两者的区别在于：**选区代码量小，且用户的意图通常就是“仅这几行”，直接提供正文对模型最准确**；
**已落盘且没有未保存修改的整个文件只提供一条链接（ACP 的 `resource_link`），由 DSH 使用自身工具读取**，
避免大文件直接占用上下文。未保存的新文件没有可读链接，已修改文件的磁盘版本也会落后于编辑器，
因此这两类文件直接携带编辑器快照；再次附加同一文件会刷新快照，不会继续发送旧内容。
...
node test/blocks.js      # 编辑器上下文拼块：选区/未保存快照、resource_link、围栏、异常数据
